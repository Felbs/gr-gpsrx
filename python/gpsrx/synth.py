...
def satellite(prn, fs, secs, doppler_hz=1234.5, code_phase_samples=700, cn0_dbhz=45.0, bits=None,
    ...
    if system == "GAL":
        from . import gale1
        n_sym = int(np.ceil(secs * 250)) + 1
        if bits is None:
            bits = rng.choice([-1.0, 1.0], n_sym)
        bits = np.asarray(bits, np.float64)
        period = np.floor(np.maximum(ph, 0) / gale1.CODE_LEN).astype(np.int64)         # one symbol per 4 ms period
        data = bits[np.minimum(period, len(bits) - 1)]
        modulated = (gale1.code_at(prn, ph, "b") * data + gale1.code_at(prn, ph, "c") * gale1.SECONDARY[period % 25]) / np.sqrt(2)
    else:
        code = code_at(prn, ph)
...
def sky(fs, secs, birds, seed=0):
    ...
    code_phase_samples, cn0_dbhz (and optionally bits, system). Returns (x, truths). Summing single-
    ...
    for b in birds:
        xi, tr = satellite(b["prn"], fs, secs, doppler_hz=b.get("doppler_hz", 0.0),
                           ...
                           bits=b.get("bits"), seed=int(rng.integers(1 << 30)), noise=False, system=b.get("system", "GPS"))
        x += xi
        truths.append(tr)
