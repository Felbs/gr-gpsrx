...
lost / idle. Slot: this channel's number; Acquisition assigns by it.""",
         ...
         params=[RATE, SLOT, {"id": "signal", "label": "Signal", "dtype": "enum", "default": "'L1CA'", "options": ["'L1CA'", "'E1'", "'E1B'"], "option_labels": ["GPS L1 C/A", "Galileo E1 (pilot-aided)", "Galileo E1-B only"]},
                 ...
                 {"id": "pll_order", "label": "PLL order after bit sync", "dtype": "enum", "default": "3", "options": ["3", "2"], "option_labels": ["3 (follows a Doppler rate: moving)", "2 (classic)"]}],
...
observable is dropped, and any observable more than 2.5 s older than the newest is ignored.""",
         make="gpsrx.pvt_solver(samp_rate=${samp_rate}, average=${average}, iono_file=${iono_file}, fix_file=${fix_file}, eph_file=${eph_file}, smoothing=${smoothing}, kf_vel_sd=${kf_vel_sd}, rinex_file=${rinex_file}, mask_deg=${mask_deg})",
...
Channel keeps up with a radio; the Python one is for replay and reading.""",
         make="gpsrx.receiver(samp_rate=${samp_rate}, n_channels=${n_channels}, interval_s=${interval_s}, "
              "threshold=${threshold}, iono_file=${iono_file}, fix_file=${fix_file}, hold=${hold}, engine=${engine}, eph_file=${eph_file}, smoothing=${smoothing}, kf_vel_sd=${kf_vel_sd}, lo_search_hz=${lo_search_hz}, n_galileo=${n_galileo}, rinex_file=${rinex_file}, mask_deg=${mask_deg})",
