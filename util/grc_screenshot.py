...
(it IS GRC's own canvas and its own File > Screen Capture code), with no window.
  ...
  python grc_screenshot.py examples/gpsrx_canvas.grc docs/img/grc_canvas.png

GRC_BLOCKS_PATH must include this repo's grc/ directory so the gpsrx blocks resolve."""
import sys
...
def main(grc_file, png):
    ...
    app = Gtk.Application(application_id="org.gnuradio.gpsrx.screenshot")
    app.register(None)
